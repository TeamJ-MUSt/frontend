...
Future<Uint8List?> fetchSongThumbnail(int songID) async {
  ...
  try {
    ...
    if (response2.statusCode == 200 &&
            response2.headers['content-type']!.startsWith('image/') ??
        false) {
...
Future<List<SearchSong>> CrawlingSongData(String query) async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception(
          'Failed to load song data due to server response: ${response.statusCode}');
    }
  } catch (e) {
    print('Failed to make request: $e');
    throw Exception(
        'Failed to process request: $e'); // Ensure you throw an exception here
  }
}
//뜻퀴즈
// 퀴즈 로직 :: 세트 조회 , 몇개 세트
...
Future<List<SeqQuiz>> getSeqQuizSet(int setNum, int songId) async {
  ...
  try {
    var response = await http.get(url);
    print(
        'Response status: ${response.statusCode}'); // Logging the response status
    if (response.statusCode == 200) {
      var decodedBody = utf8.decode(response.bodyBytes);
      print(
          'Response body: $decodedBody'); // Logging the response body for debugging
      return SeqQuizFromJson(
          decodedBody); // Ensure this function is defined correctly
    } else {
      print(
          'Failed to load quiz data with status code: ${response.statusCode}');
...
Future<quizSet> fetchQuizData(int songId, String quizType) async {
  var url =
      Uri.parse('http://${ip}/quiz/info?songId=${songId}&type=${quizType}');
  try {
    var response = await http.get(url);
    if (response.statusCode == 200) {
...
Future<List<SongWord>> getSongWordbook(int songId) async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      var decodedBody = utf8.decode(response.bodyBytes);
      return WordParser.parseMusicWordList(
          decodedBody); // Parse the JSON correctly
    } else {
      throw Exception(
          'Failed to load word data with status code: ${response.statusCode}');
    }
